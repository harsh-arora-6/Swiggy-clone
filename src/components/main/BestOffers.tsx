...
export default function BestOffers() {
  ...
  const findLeftScrollDist = () => {
    return contentRef.current!.scrollLeft;
  };

  const findRightScrollDist = () => {
    const nodeWidth = contentRef.current!.clientWidth;
    const nodeScrollWidth = contentRef.current!.scrollWidth;
    const nodeScrollLeft = contentRef.current!.scrollLeft;
    return nodeScrollWidth - nodeWidth - nodeScrollLeft;
  };
  const scrollToLeft = () => {
    const dist = findLeftScrollDist();
    // scroll completely left if distance is less than 450 as
    // if we subtract less 400 then button will get disabled
    if (dist <= 445) {
      contentRef.current!.scrollLeft -= dist;
    } else {
      contentRef.current!.scrollLeft -= 400;
    }
  };
  const scrollToRight = () => {
    const dist = findRightScrollDist();
    if (dist <= 445) {
      contentRef.current!.scrollLeft += dist;
    } else {
      contentRef.current!.scrollLeft += 400;
  ...
  const updateBtnStatus = throttle(() => {
    if (findLeftScrollDist() <= 45) {
      setLeftBtnDisabled(true);
    } else {
      setLeftBtnDisabled(false);
    }
    if (findRightScrollDist() <= 45) {
  ...
  return (
    <section className="content-section best-offers">
      <div className="content-wrapper">
        ...
        <div className="content" ref={contentRef} onScroll={updateBtnStatus}>
          {imageLinks.map((imgLink, index) => (
            <BestOfferCard key={index} src={imgLink} />
          ))}
        </div>
      </div>
    </section>
